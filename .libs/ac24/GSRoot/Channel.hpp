...
struct TestPersistentTypeOf {
	template <class T>
	static decltype (PersistentTypeOf (std::declval<T> ()))		Test (int);
...
template <class T>
using	ExternalPersistentType = decltype (Imp::TestPersistentTypeOf::Test<T> (0));

template <class T>
constexpr bool	HasExternalPersistentTypeOf = !IsSame<std::false_type, ExternalPersistentType<T>>;
...
public:
	...
	template <class Type, EnableIf<!IsObjectType<Type> && !HasExternalPersistentTypeOf<Type>, bool> = true>
...
template <class Type, EnableIf<!IsObjectType<Type> && !HasExternalPersistentTypeOf<Type>, bool>>
...
template <class Type>
constexpr bool	HasInternalWriteMethod (decltype (GSErrCode { std::declval<const Type> ().Write (std::declval<GS::OChannel&> ()) })*) { return true; }
