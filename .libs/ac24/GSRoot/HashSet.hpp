...
template <class Type>
class HashSet : public HashableContainer<Type> {
private:
	struct HashEntry {
...
public:
   ...
   ~HashSet ();
	...
	bool					Get (const Type& item, Type** result);
...
template <class Type>
bool	HashSet<Type>::Get (const Type& item, Type** result)
{
	NULL_REF_CHECK (item);
	NULL_PTR_CHECK (result);
...
void	HashSet<Type>::Rehash (USize newHashListTableSize)
{
	...
	for (UIndex i = 0; i < hashEntryCount; i++) {
		...
		UIndex newHashListIndex = GenerateHashValue (hashEntry.item) % newHashListCount;
...
template <class Type>
HashSetInserter<Type>	GetInserter (HashSet<Type>& destination)
{
