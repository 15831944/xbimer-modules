...
public:
	...
	Vector3D	GetNormalVector () const { return { data[6], data[7], data[8] }; }
};
...
struct TreeData {
	KDPoly							kdPoly;
	KDPGON							pgon;
	GS::Optional<GS::Array<Coord>>	triangulatedCoords;

	TreeData () : kdPoly (), pgon () {}
	TreeData (const KDPoly& kdPoly, const KDPGON& pgon) : kdPoly (kdPoly), pgon (pgon), triangulatedCoords (GS::NoValue) {}

	Vector3D					GetPolygonNormal () const { return kdPoly.GetNormalVector ();  }
	const GS::Array<Coord>&		GetTriangulatedCoords () { return *triangulatedCoords;  }
	bool						HasTriangulatedCoords () { return triangulatedCoords.HasValue (); }
	void						SetTriangulatedCoords (GS::Array<Coord>&& coords) { triangulatedCoords = std::move (coords); }
};
...
typedef std::function<UInt32 (const Modeler::Model3DViewer& viewer, const GS::HashTable<UIndex, bool> *elimElems)> ValidPolygonEnumeratorFunction;
