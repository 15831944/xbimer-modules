...
#include "DynamicValueTypeTemplates.hpp"
...
public:
	...
	virtual bool NeedToProcess (const GS::DynamicValue* enumKey) const = 0;
	virtual bool NeedToProcess (const GS::DynamicValue* enumKey, const PropertyOwner& propertyOwner) const = 0;
};
...
public:
	...
	virtual bool NeedToProcess (const GS::DynamicValue* enumKey) const override;
	virtual bool NeedToProcess (const GS::DynamicValue* enumKey, const PropertyOwner& propertyOwner) const override;
...
template <typename ValueType>
bool Property::ValueBasedEnumerationValueFilter<ValueType>::NeedToProcess (const GS::DynamicValue* enumKey) const
{
	typedef typename GS::GetSingleVariantType<ValueType>::Type VariantType;
	if (DBERROR (!GS::DynamicValue::IsType<VariantType> (enumKey))) {
		return false;
	}
...
template <typename ValueType>
bool Property::ValueBasedEnumerationValueFilter<ValueType>::NeedToProcess (const GS::DynamicValue* enumKey, const PropertyOwner& /*propertyOwner*/) const
{
