...
public:
	...
	private:
		...
		GS::Optional<GS::String>	shortName;			// example: UInt16    <- optional, if it is empty then name is used instead
		GS::Optional<GS::String>	fundamentalName;	// example: unsigned short (type without typedefs)  <- optional, if it is empty then name is used instead
...
public:
	...
	virtual UInt32	GetAlignment (void) const;
