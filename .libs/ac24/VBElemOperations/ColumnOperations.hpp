...
public:
	...
	static UnitVector_3D GetColumnDirection (const VBElem::ColumnConstRef& column);

	static Vector3D GetSegmentVector (const VBElem::ColumnSegmentConstRef& segment);

	static Box3D GetRawBounds3D (const VBElem::ColumnConstRef& column);
	static Box3D GetRawBounds3D (const VBElem::ColumnSegmentConstRef& column);

	static Geometry::GeneralCoordinateSystem3D GetProfileCoordinateSystem (VBElem::ColumnSegmentConstRef segment, VBElemOperations::RefPointLocation location);
